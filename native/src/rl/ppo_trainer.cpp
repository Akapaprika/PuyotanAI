...
// Bytes per environment observation: [kObsPlayers * kObsColors * kObsCols *
// kObsRows]
...
CppPPOTrainer::CppPPOTrainer(int num_envs, int num_steps,
                             const std::string& arch, int hidden_dim,
                             uint32_t base_seed, PPOConfig cfg,
                             const std::map<std::string, int>& arch_params)
    : env_(num_envs, base_seed), buffer_(num_steps, num_envs, torch::kCPU),
      cfg_(cfg), num_envs_(num_envs), num_steps_(num_steps), arch_(arch),
      hidden_dim_(hidden_dim), arch_params_(arch_params),
      episode_scores_(torch::zeros({num_envs}, torch::kFloat32)),
      episode_game_scores_(torch::zeros({num_envs}, torch::kFloat32)),
      act_p1_buf_(num_envs), act_p2_buf_(num_envs), rew_buf_(num_envs),
      done_buf_(num_envs), chain_buf_(num_envs), score_buf_(num_envs),
    ...
    if (arch == "mlp") {
        ...
        policy_ =
            std::make_unique<ResNetPolicyWrapper>(hidden_dim, arch_params_);
    } else {
        throw std::invalid_argument("Unknown arch: " + arch +
                                    " (expected 'mlp', 'cnn' or 'resnet')");
    }
    policy_->train(true);
    // Build optimizer
    optimizer_ = std::make_unique<torch::optim::Adam>(
        policy_->parameters(), torch::optim::AdamOptions(cfg_.lr).eps(1e-5));
    ...
    // [OPTIMIZATION] ZERO-COPY initialization: from_blob refers to obs_buf_
    // directly.
    curr_obs_ = torch::from_blob(
        obs_buf_.data(),
        {num_envs, kObsPlayers, kObsColors, kObsCols, kObsRows}, torch::kUInt8);
}
// ---------------------------------------------------------------------------
// Public: trainStep
// ---------------------------------------------------------------------------
TrainMetrics CppPPOTrainer::trainStep(bool p2_random) {
    auto [max_chain, avg_max_chain, chain_rate, avg_reward, avg_game_score,
          avg_game_len, avg_max_potential] = collectRollouts_(p2_random);
    float loss = ppoUpdate_();
    return TrainMetrics{loss,          avg_reward,       max_chain,
                        avg_max_chain, chain_rate,       avg_game_score,
                        avg_game_len,  avg_max_potential};
}
// ---------------------------------------------------------------------------
// Public: save / load
// ---------------------------------------------------------------------------
...
void CppPPOTrainer::loadP2(const std::string& path) {
    if (arch_ == "mlp") {
        ...
        opp_policy_ =
            std::make_unique<CNNPolicyWrapper>(hidden_dim_, arch_params_);
    } else if (arch_ == "resnet") {
        opp_policy_ =
            std::make_unique<ResNetPolicyWrapper>(hidden_dim_, arch_params_);
    } else {
...
// ---------------------------------------------------------------------------
std::tuple<int, float, float, float, float, float, float>
CppPPOTrainer::collectRollouts_(bool p2_random) {
    policy_->train(false);
    int max_chain = 0;
    ...
    uint64_t prng_state =
        env_.getMatch(0).getTsumo().getSeed() + 0x123456789ULL;
    ...
    for (int t = 0; t < num_steps_; ++t) {
        ...
        env_.stepNative(sp_p1, sp_p2, sp_rew, sp_done, sp_chain, sp_score,
                        sp_pot, sp_obs);

        // [OPTIMIZATION] ZERO-COPY: from_blob without clone() allows
        // RolloutBuffer to copy directly.
        auto rewards_t =
            torch::from_blob(sp_rew.data(), {num_envs_}, torch::kFloat32);
        auto dones_t =
            torch::from_blob(sp_done.data(), {num_envs_}, torch::kFloat32);

        buffer_.storeStep(t, out.actions, out.log_probs, out.values, rewards_t,
                          dones_t);
        // [OPTIMIZATION] Unified single loop + Raw Pointers for maximum Cache
        // Line efficiency.
        {
            ...
            for (int i = 0; i < num_envs_; ++i) {
                ...
                episode_lengths_[i]++;
                max_potential_per_env_[i] = std::max(max_potential_per_env_[i],
                                                     static_cast<int>(pacc[i]));

                // 2. Check done: record per-episode stats then reset
                // accumulators
                if (dacc[i] > 0.5f) {
                    ...
                    completed_max_potentials_.push_back(
                        max_potential_per_env_[i]);
                    sacc[i] = 0.0f;
                    gsacc[i] = 0.0f;
    ...
    if (!completed_max_chains_.empty()) {
        ...
        chain_rate = static_cast<float>(chained_episodes) /
                     static_cast<float>(completed_max_chains_.size());
        if (chained_episodes > 0) {
            avg_max_chain = static_cast<float>(sum_chains) /
                            static_cast<float>(chained_episodes);
        }
    } else {
        int chained_envs = 0;
        int sum_chains = 0;
        ...
        chain_rate =
            static_cast<float>(chained_envs) / static_cast<float>(num_envs_);
        if (chained_envs > 0) {
            avg_max_chain = static_cast<float>(sum_chains) /
                            static_cast<float>(chained_envs);
        }
    }

    float avg_reward = sum_reward / static_cast<float>(num_steps_ * num_envs_);

    // Calculate the mean of completed episode final scores. If no episodes
    // completed during this iteration, fallback to the mean of ongoing accumulated scores.
    float avg_game_score = 0.0f;
    if (!completed_scores_.empty()) {
        float sum_completed = std::accumulate(completed_scores_.begin(),
                                              completed_scores_.end(), 0.0f);
        avg_game_score = sum_completed / static_cast<float>(completed_scores_.size());
    } else {
        float ongoing_sum = episode_game_scores_.sum().item<float>();
        avg_game_score = ongoing_sum / static_cast<float>(num_envs_);
    }

    float avg_game_len =
        completed_lengths_.empty()
            ? static_cast<float>(num_steps_)
            : static_cast<float>(std::accumulate(completed_lengths_.begin(),
                                                 completed_lengths_.end(), 0)) /
                  static_cast<float>(completed_lengths_.size());

    float avg_max_potential =
        completed_max_potentials_.empty()
            ? 0.0f
            : static_cast<float>(
                  std::accumulate(completed_max_potentials_.begin(),
                                  completed_max_potentials_.end(), 0)) /
                  static_cast<float>(completed_max_potentials_.size());

    return {max_chain,      avg_max_chain, chain_rate,       avg_reward,
            avg_game_score, avg_game_len,  avg_max_potential};
}
// ---------------------------------------------------------------------------
// Private: ppoUpdate_
// ---------------------------------------------------------------------------
float CppPPOTrainer::ppoUpdate_() {
    ...
    b_advantages =
        (b_advantages - b_advantages.mean()) / (b_advantages.std() + 1e-8f);
    int total = buffer_.totalSteps();
    // [OPTIMIZATION] Reuse indices_buf_ to avoid allocating a [total] tensor on
    // the heap every epoch.
    torch::randperm_out(indices_buf_, total);

    float total_loss = 0.0f;
    for (int epoch = 0; epoch < cfg_.num_epochs; ++epoch) {
        if (epoch > 0) {
            torch::randperm_out(indices_buf_,
                                total); // Reshuffle for next epochs
        }
        for (int start = 0; start < total; start += cfg_.minibatch) {
            ...
            total_loss += updateMinibatch_(
                b_obs.index({idx}), b_actions.index({idx}),
                b_log_probs.index({idx}), b_advantages.index({idx}),
                b_returns.index({idx}));
        }
    }
    return total_loss;
...
// ---------------------------------------------------------------------------
float CppPPOTrainer::updateMinibatch_(const torch::Tensor& b_obs,
    ...
    auto clip_r =
        torch::clamp(ratio, 1.0f - cfg_.clip_eps, 1.0f + cfg_.clip_eps);
    auto loss_pi =
        -torch::min(ratio * b_advantages, clip_r * b_advantages).mean();
    auto loss_v = 0.5f * ((out.values - b_returns).pow(2)).mean();
    auto loss_ent = -out.entropies.mean();
    auto loss =
        loss_pi + cfg_.value_coef * loss_v + cfg_.entropy_coef * loss_ent;
