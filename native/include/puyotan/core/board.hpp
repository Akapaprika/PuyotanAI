...
inline constexpr auto kPlacementMask = []() consteval {
    std::array<std::array<BitBoard, 16>, 6> arr{};
    for (int x = 0; x < 6; ++x) {
        for (int y = 0; y < 16; ++y) {
            uint64_t lo = 0;
            uint64_t hi = 0;
            ...
            const uint64_t keep_mask =
                static_cast<uint64_t>(config::Board::kVisibleColMask)
                << col_shift;
...
class Board {
  public:
    ...
    inline void dropNewPieceFast(int x, int y, Cell color) noexcept {
        [[assume(x >= 0 && x < 6)]];
        [[assume(y >= 0 && y < 16)]];
        assert(toIndex(color) >= 0 &&
               toIndex(color) < config::Board::kNumColors);

        const BitBoard& mask = kPlacementMask[x][y];
        boards_[toIndex(color)] |= mask;
        occupancy_ |= mask;
    }
