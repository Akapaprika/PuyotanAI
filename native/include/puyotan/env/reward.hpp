...
struct RewardContext {
    ...
    int p1_chain_count = 0;    ///< Chains completed this step
    int p1_total_erased = 0;   ///< Total puyos erased in the chain sequence
    int p1_ojama_sent = 0;     ///< Ojama puyos sent to opponent
    bool p1_all_clear = false; ///< True if the board was fully cleared

    int p1_puyo_count = 0;
    int p1_connectivity_score = 0;
    int p1_isolated_puyo_count = 0;
    int p1_near_group_count = 0;
    int p1_death_col_height = 0;
    float p1_height_variance = 0.0f;
...
struct RewardWeights {
    ...
    struct Performance {
        ...
        float chain_scale = 0.0f;
        float chain_power = 2.0f;
        float min_chain_threshold = 0.0f;
        float premature_chain_penalty = 0.0f;
        float all_clear_bonus = 0.0f;
        float erasure_count_scale = 0.0f;
        float ojama_sent_scale = 0.0f;
    } performance;
    ...
    struct Board {
        ...
        float near_group_bonus = 0.0f;
        float height_variance_penalty = 0.0f;
    ...
    void from_json(const json& j) {
        ...
        if (j.contains("performance")) {
            ...
            get(p, "chain_bonus_scale", performance.chain_scale);
  ...
  public:
    ...
    float calculate(const RewardContext& ctx, int player_id) const {
        ...
        if (ctx.status != MatchStatus::Playing) {
            const bool win = (is_p1 && ctx.status == MatchStatus::WinP1) || (!is_p1 && ctx.status == MatchStatus::WinP2);
            const bool loss = (is_p1 && ctx.status == MatchStatus::WinP2) || (!is_p1 && ctx.status == MatchStatus::WinP1);
            if (win)
                r += weights.match.win;
            else if (loss)
                r += weights.match.loss;
            else
                r += weights.match.draw;
        }
        ...
        if (s_chain > 0) {
            if (static_cast<float>(s_chain) >= weights.performance.min_chain_threshold) {
                r += std::pow(static_cast<float>(s_chain), weights.performance.chain_power) * weights.performance.chain_scale;
            } else {
        ...
        // Performance
        if (s_all_clear)
            r += weights.performance.all_clear_bonus;
        ...
        r += static_cast<float>(s_score - (is_p1 ? ctx.p2_delta_score : ctx.p1_delta_score)) * weights.opponent.ojama_diff_scale;
