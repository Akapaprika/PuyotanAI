...
def run_deep_search_experiment(beam_width=3000, look_ahead=10, num_games=20):
    ...
    print(f"Player 1 (New AI: Gaze & Sub-chain ON) : {p1_wins} wins ({(p1_wins/num_games)*100:.1f}%)")
    print(f"Player 2 (Old AI: Gaze & Sub-chain OFF): {p2_wins} wins ({(p2_wins/num_games)*100:.1f}%)")
    print(f"Average Frames per Game: {avg_frames:.1f}")
    print(f"Max Chain: P1={max_chain_p1}, P2={max_chain_p2}")

if __name__ == "__main__":
    run_deep_search_experiment(3000, 10, 20)
